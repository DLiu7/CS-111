...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        int numOfProfiles = Integer.parseInt(StdIn.readLine());

        //Allocated memory to story the new databse data
        ...
        //Goes through txt file to read and store data as profile object
        while(!StdIn.isEmpty()){
            String name = StdIn.readLine();
            String seq1 = StdIn.readLine();
            String seq2 = StdIn.readLine();

            Profile person = new Profile(name, S1_STRs, S2_STRs, seq1, seq2);
    ...
    public void readSTRsOfInterest ( String filename ) {
        ...
        //Reads first line containing quantity of STRs
        int num = Integer.parseInt(StdIn.readLine());

        //Allocates memory to store the STRs 
        STRsOfInterest = new String[num];

        //Iterating through and storing data into STRsOfInterest depending on the txt file inputted
        for(int i = 0; i < num; i++){
	...
	/* WRITE YOUR CODE HERE */
    Profile unknownProfile = null;
        
    //Reads the two lines from the txt file
        String sequence1 = StdIn.readLine();
        String sequence2 = StdIn.readLine();
    
    //Creates profile object using given information
    unknownProfile = new Profile("Unknown", null, null, sequence1, sequence2);
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        //One variable to store current number of STR repeats and the other to store all time highest repeat times
        int numRepeat = 0;
        int maxNumRepeat = 0;

        //For loop checks for STR in a sequence without going past sequence length
        for (int i = 0; i <= sequence.length() - STR.length(); i++) {

            //Adding to number of STR Repeats
            if (sequence.substring(i, i + STR.length()).equals(STR)) {
                numRepeat++;

            //Repeat ends, current vs all time repeats are compared and the largest remains
            } else {
                maxNumRepeat = Math.max(maxNumRepeat, numRepeat);
                numRepeat = 0;
            }
        }

        //Compares largest to ensure the last check is or is not greater than previous repeats
        maxNumRepeat = Math.max(maxNumRepeat, numRepeat);

        //Creates a new STR object using data
        STR newSTR = new STR(STR, maxNumRepeat);
    ...
    public void createProfileSTRs ( Profile profile, String[] allSTRs ) {
        ...
        //Caller methods in profile class gives access to sequences that are store locally as strings
        String dnaSequence = profile.getSequence1();
        String dnaSequence2 = profile.getSequence2();
        
        //Object arrays to store STRs in each sequence
        STR[] S1_STRs = new STR[allSTRs.length];
        STR[] S2_STRs = new STR[allSTRs.length];

        //For loop iterates through all the STRs that will be searched for in each seqeunce
        for (int i = 0; i < allSTRs.length; i++) {
            String str = allSTRs[i];
    
            //Calls findSTRInSequence method to find highest number of STR repeats in a sequence
            STR strObject1 = findSTRInSequence(dnaSequence, str);
            STR strObject2 = findSTRInSequence(dnaSequence2, str);
            
            //Highest number of STR repeats from each sequence is stored into object array
            S1_STRs[i] = strObject1;
            S2_STRs[i] = strObject2;
        }

        //Calls setter method that only takes object arrays to set STRs for the parameter profile
    ...
    public boolean identicalSTRs ( STR[] s1, STR[] s2 ) {
        ...
        //Checking if object arrays are the same size
        if(s1.length != s2.length){
            return false;
        }

        //Checking the entirety of the first object array and comparing to s2. If any discrepancies occur, the two do not match.
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        //Creates a new object ArrayList to hold matching profiles to the unknown profile
        ArrayList<Profile> match = new ArrayList<>();

        //For loop going through all the profiles in the object array
        for (int i = 0; i < database.length; i++) {
            //Current profile
            Profile cP = database[i];

            //Calls identicalSTRs method to check if current profile and unknown's STRs match
            if (identicalSTRs(cP.getS1_STRs(), unknownProfileS1_STRs)) {
                match.add(cP);
            }
        }
